...
INIFile::INIFile() : Name("$EverywhereMediumFont"), SingleDamageFront(""), SingleDamageBack(""), TimeDamageFront(""), TimeDamageBack(""), BlockString("BLOCK"), SneakString("SNEAK"), CriticalString("CRITICAL"),
HealthHealFront(""), HealthHealBack(""), MagickaDamageFront(""), MagickaDamageBack(""), MagickaHealFront(""), MagickaHealBack(""), StaminaDamageFront(""), StaminaDamageBack(""), StaminaHealFront(""),
StaminaHealBack(""), EnablePlayer(true), HideRegen(true), HideNoLOS(false), OnlyHostile(false), Size(24), Alpha(100), BlockSize(24), SneakSize(24), CriticalSize(24), HostileSingleDamage(0xFFFFFF),
HostileTimeDamage(0xFFD700), FollowerSingleDamage(0xFF0000), FollowerTimeDamage(0x800080), Block(0xFF8C00), Sneak(0xFF8C00), Critical(0xFF8C00), HealthHeal(0x00FF00), MagickaDamage(0x0000FF),
MagickaHeal(0xFF00FF), StaminaDamage(0x008000), StaminaHeal(0x00FFFF), PositionX(0.5f), PositionY(0.75f), DamageDisplayMode(0), HealDisplayMode(0), DisplayDispersion(0.02), TimeDamageDifference(3),
TimeDamageInterval(60 + 1), MinHeal(8), EnableBlock(1), EnableSneak(1), EnableCritical(1), EnableHealth(3), EnableMagicka(0), EnableStamina(0), NearRadius(std::pow(7000, 2.0)),
FarRadius(std::pow(8000, 2.0)), ListUpdateTime(100)
{
	...
	for (std::string str : *list)
	{
		...
		if (!key.empty())
		{
			...
			else
			{
				...
				{
					_MESSAGE("%s : invalid argument", (vec.at(0)).c_str());
					_MESSAGE("   %s", e.what());
				}
				catch (const std::out_of_range& e)
				{
					_MESSAGE("%s : out of range", (vec.at(0)).c_str());
					_MESSAGE("   %s", e.what());
				}
